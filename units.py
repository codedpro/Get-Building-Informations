...
# Load unique building IDs from buildings_array.json
...
with open(buildings_file, 'r', encoding='utf-8') as f:
    ...
    for building in buildings_data:
        building_id = building.get('id')  # Use 'id' or 'buildingid' as appropriate
        if building_id is not None:
            building_ids.add(building_id)

# Output file for units data
...
# Function to fetch units data synchronously
...
def main():
    units_data = []
    unit_ids = set()
    max_workers = 1  # Limit the number of concurrent threads to 50
